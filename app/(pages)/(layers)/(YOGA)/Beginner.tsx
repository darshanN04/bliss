...
const Beginner: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAsanas = async () => {
      const { data, error } = await supabase.rpc('get_beginners_yoga'); // Replace with your function name
  ...
  return (
    <LinearGradient colors={['rgb(168, 213, 186)', 'rgb(255, 216, 182)']} style={{ flex: 1 }}>
              ...
              <View style={styles.modalView}>
                <LinearGradient colors={['#fff', '#fff']} style={{ flex: 1 }}>
                  ...
                  <ScrollView contentContainerStyle={styles.modalContent}>
                    ...
                    <Text style={styles.modalTextOverlay}>{"\nTARGETS"}</Text>
                    <Text style={styles.textOverlay}>
                      {asana.target.split(', ').map((line, index) => (
                        <Text key={index} style={styles.listItem}>
                          • {line.trim()}{'\n'}
                        </Text>
                      ))}
...
const styles = StyleSheet.create({
  ...
  textOverlay: {
    ...
    // textShadowOffset: { width: 0, height: 0 }, // Center the shadow
    // textShadowRadius: 0, // Adjust thickness
  },
  listItem: {
    fontSize: 16,
    marginLeft: 5,
